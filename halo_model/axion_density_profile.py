...
def func_core_radius(M, cosmo_dic):
    ...
    r_vir = func_r_vir(cosmo_dic['z'], (1 + cosmo_dic['omega_ax_0']/cosmo_dic['omega_db_0']) * M, cosmo_dic['Omega_m_0'],  cosmo_dic['Omega_m_0'],  cosmo_dic['Omega_w_0']) / cosmo_dic['h'] * 3.086e+22 # in m
...
def func_dens_profile_ax(r_arr, M, cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param, hmcode_params, eta_given=False, axion_dic=None):
    """
        ...
        if rho_central_param == 0 or M_cut > M:
            ...
            NFW = cosmo_dic['omega_ax_0']/cosmo_dic['omega_db_0'] * \
                          NFW_profile(M, r_arr, power_spec_dic_sigma['k'], power_spec_dic_sigma['cold'], cosmo_dic, 
                                      hmcode_params, cosmo_dic['Omega_db_0'], cosmo_dic['Omega_db_0'], eta_given = eta_given, axion_dic=axion_dic)
...
def func_ax_halo_mass(M, cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param, hmcode_params, eta_given=False, axion_dic=None):
    """
    ...
    if isinstance(M, (int, float)) == True:
        ...
        integrand = func_dens_profile_ax(r_arr, M, cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param, hmcode_params, eta_given=eta_given, axion_dic=axion_dic) * r_arr**2
        ...
        for i in range(len(M)):
            ...
            integral[i] = 4 * np.pi * integrate.simps(y=func_dens_profile_ax(R_int, M[i], cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param[i], hmcode_params, eta_given=eta_given, axion_dic=axion_dic)*R_int**2, x=R_int)
            
        return integral

        #return [4 * np.pi * integrate.simps(y=func_dens_profile_ax(np.geomspace(1e-15, func_r_vir(M[i], cosmo_dic, cosmo_dic['Omega_db_0']), num=100), 
        #                                                                      M[i], cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param[i], eta_given=eta_given) * \
        #                                    np.geomspace(1e-15, func_r_vir(M[i], cosmo_dic, cosmo_dic['Omega_db_0']), num=100)**2, 
        #                                    x=np.geomspace(1e-15, func_r_vir(M[i], cosmo_dic, cosmo_dic['Omega_db_0']), num=100)) for i in range(len(M))]
        

def func_central_density_param(M, cosmo_dic, power_spec_dic_sigma, M_cut, eta_given=False, axion_dic=None):
    """
    M and M_cut in solar_mass/h
    ...
    if isinstance(M, (int, float)) == True:
        if M < M_cut:
            ...
            def integrand_ax(x):
                return func_dens_profile_ax(x, M, cosmo_dic, power_spec_dic_sigma, M_cut, 1., eta_given=eta_given, axion_dic=axion_dic)*x**2
            ...
            def func_find_root(dens):
                return func_ax_halo_mass(M, cosmo_dic, power_spec_dic_sigma, M_cut, dens, hmcode_params, eta_given=eta_given, axion_dic=axion_dic) - cosmo_dic['Omega_ax_0']/cosmo_dic['Omega_db_0'] * M
        ...
        for idx, m in enumerate(M):
            if m < M_cut:
                ...
                #so we need the soliton and NFW part
                def integrand_ax(x):
                    return func_dens_profile_ax(x, m, cosmo_dic, power_spec_dic_sigma, M_cut, 1., hmcode_params, eta_given=eta_given, axion_dic=axion_dic)*x**2
                ...
                #print(dens_param, dens_param_test)
...
def func_dens_profile_ax_kspace(k, M, cosmo_dic, power_spec_dic_sigma, M_cut, central_dens_param, hmcode_params, eta_given=False, axion_dic=None):
    """
    ...
    M_ax = func_ax_halo_mass(M, cosmo_dic, power_spec_dic_sigma, M_cut, central_dens_param, hmcode_params, axion_dic=axion_dic)
    r_vir = func_r_vir(cosmo_dic['z'], M, cosmo_dic['Omega_db_0'], cosmo_dic['Omega_m_0'], cosmo_dic['Omega_w_0'])
    hmcode_params = HMCode_param_dic(cosmo_dic, power_spec_dic_sigma['k'], power_spec_dic_sigma['cold']) 
    ...
    if isinstance(M, (int, float)) == True:
        r_arr = np.geomspace(1e-15, r_vir, num=2000)
        dens_profile_arr = func_dens_profile_ax(r_arr, M, cosmo_dic, power_spec_dic_sigma, M_cut, central_dens_param, hmcode_params, eta_given=eta_given, axion_dic=axion_dic) \
        ...
        for idx, m in enumerate(M):
            if  M_ax[idx] == 0:
                ...
                dens_profile_arr = func_dens_profile_ax(r_arr, m, cosmo_dic, power_spec_dic_sigma, M_cut, central_dens_param[idx], hmcode_params, eta_given=eta_given, axion_dic=axion_dic) \
                                   * r_arr**2 * np.sin(np.outer(k, r_arr)) / np.outer(k, r_arr)
